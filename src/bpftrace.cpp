                  ...
                  BpfBytecode bytecode)
{
  ...
  if (run_tests_ || run_benchmarks_) {
    if (run_tests_) {
      ...
      std::stringstream ss;
      auto test_output = std::make_unique<output::TextOutput>(ss);
      ...
      for (size_t index = 0; index < resources.test_probes.size(); index++) {
        ...
        bool passed = opts.retval == 0;
        poll_output(*test_output, true);
        out.test_result(all_tests, index, passed, ss.str());
        ss.str("");

        // Override the return value.
        if (rval == 0 && opts.retval != 0) {
          rval = 1;
        }
      }
      ...
      for (size_t index = 0; index < resources.benchmark_probes.size();
        ...
        out.benchmark_result(all_benches, index, opts.duration);
      }
    }
  } else {
